...
if(!$dbconnect)
{
	...
	if(isset($_POST['verified']))
	{
		...
		if($expert_id1 != NULL)
		{
			$sql = "UPDATE questions SET subject = '$updatesubject' , topic = '$updatetopic' , question_field = '$updatequestion' , answer_field = '$updateanswer' , approach = '$updateapproach' , difficulty = '$updatedifficulty' , grade = '$updategrade' , expert_id2 = '$emails' , verified = verified + 1 WHERE question_id = '$details' ";
			$result = mysqli_query($con,$sql);
			header('location: expertdashboard.php');

		}
		$sql7 = "UPDATE expert SET verified_count = verified_count + 1 where expert_id = '$emails'";
	...
	if(isset($_POST['fail']))
	{
		if($expert_id1 == NULL)
		{
			$sql = "UPDATE questions SET verified = verified - 1 , expert_id1 = '$email' WHERE question_id = '$details'";
			$result = mysqli_query($con,$sql);
			header('location: expertdashboard.php');
		}
		if($expert_id2 != NULL)
		{
			$sql = "UPDATE questions SET verified = verified - 1 , expert_id2 = '$email' WHERE question_id = '$details'";
			$result = mysqli_query($con,$sql);
			header('location: expertdashboard.php');
		}
	}
...
<div id="frm-cvr">
			...
			<form method = "POST" autocomplete="off" enctype="multipart/form-data">
				...
				<div class="f-i-bx b3 mrg3b">
					<div class="tb">
						<div class="td icn"><i class="material-icons">person</i></div>
						<div class="td prt"><input type="text" name = "updatedifficulty" placeholder = "Please enter Easy/Medium/Hard only" required></div>
					</div>
				</div>
				...
				<div class="f-i-bx b3 mrg3b">
					<div class="tb">
						<div class="td icn"><i class="material-icons">person</i></div>
						<div class="td prt"><input type="text" name = "updategrade" placeholder = "Ex: 6th, 7th, B. Tech" required></div>
					</div>
				</div>
				<div id="s-btn" class="mrg25t"><input type="submit" value="Verified" class="b3" name = 'verified' ></div>

				<div id="s-btn" class="mrg25t"><input type="submit" value="Fail" class="b3" name = 'fail' ></div>
